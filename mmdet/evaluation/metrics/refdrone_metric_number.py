...
class RefDroneMetricNumber(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        for data_sample in data_samples:
            ...
            result['numbers'] = pred['numbers'].cpu()
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for result in results:
            ...
            number_results = result['numbers'][0]
            

            num_prediction = filtered_boxes.shape[0]
            num_gt = gt_bbox_all.shape[0]
            copy_num_gt = np.clip(num_gt, 0, 4)
            print(number_results, copy_num_gt)
            if num_prediction != 1:
                orginal_one += 1
            ...
            if no_target_flag:
                ...
                if number_results == copy_num_gt:
                    number_acc += 1                    

                else:
                    number_mae += abs(number_results-copy_num_gt)
        ...
        print(instance_nt)
        print(image_nt)
        print(number_acc, number_mae/len(results))
        print(results_acc, results_zero_acc, number_one, results_one_acc, orginal_one)
        results = {
