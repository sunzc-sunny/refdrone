...
from llava.conversation import conv_vicuna_object, conv_vicuna_v1, conv_templates
...
@MODELS.register_module()
class LLavaDetDataPreprocessor(ImgDataPreprocessor):
    def __init__(self,
                 mean: Sequence[Number] = None,
                 std: Sequence[Number] = None,
                 pad_size_divisor: int = 1,
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if data_samples is not None:
            ...
            for data_sample, pad_shape, org_image in zip(data_samples, batch_pad_shape, org_images):
                data_sample.set_metainfo({
                    'batch_input_shape': batch_input_shape,
                    'pad_shape': pad_shape
                })
        ...
        for data_sample in data_samples:
            ...
            qs = data_sample.text.strip()
            if qs.endswith(" ."):
                qs = qs[:-2]
                qs = qs.strip()
            if not qs.endswith("."):
                qs += "."
            qs = "<image>\n[refer] Give me the location of <p> " + qs + " </p>"
...
@MODELS.register_module()
class LLavaDetCountDataPreprocessor(ImgDataPreprocessor):
    def __init__(self,
                 mean: Sequence[Number] = None,
                 std: Sequence[Number] = None,
                 pad_size_divisor: int = 1,
