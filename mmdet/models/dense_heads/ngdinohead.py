...
class NGDINOHead(DINOHead):
    ...
    def _init_layers(self) -> None:
        ...
        # 新增的单值回归分支
        single_value_branch = []
        for _ in range(self.num_reg_fcs):  # 可以使用相同的层数，或者自定义
            single_value_branch.append(Linear(self.embed_dims, self.embed_dims))
            single_value_branch.append(nn.ReLU())
        single_value_branch.append(Linear(self.embed_dims, 1))  # 最后输出一个值
        single_value_branch = nn.Sequential(*single_value_branch)
        ...
        if self.share_pred_layer:
            ...
            self.single_value_branches = nn.ModuleList(
                [single_value_branch for _ in range(self.num_pred_layer-1)])
        else:
            self.cls_branches = nn.ModuleList(
            ...
            self.single_value_branches = nn.ModuleList([
                copy.deepcopy(single_value_branch) for _ in range(self.num_pred_layer-1)
            ])
        # branch = self.single_value_branches[-1]
        # branch[-1].bias = None
        # self.init_single_weights()

    def init_single_weights(self):
        for m in self.single_value_branches:
    ...
    def forward(
        ...
        all_layers_outputs_numes = []
        for layer_id in range(hidden_states.shape[0]):
            reference = inverse_sigmoid(references[layer_id])
            reference_num = inverse_sigmoid(references_num[layer_id])
            ...
            tmp_num_preds = self.single_value_branches[layer_id](hidden_state)
            ...
            outputs_num = tmp_num_preds.sigmoid()
            all_layers_outputs_numes.append(outputs_num)

        all_layers_outputs_classes = torch.stack(all_layers_outputs_classes)
        all_layers_outputs_coords = torch.stack(all_layers_outputs_coords)
        all_layers_outputs_numes = torch.stack(all_layers_outputs_numes)


        return all_layers_outputs_classes, all_layers_outputs_coords, all_layers_outputs_numes
    ...
    def predict(self,
        ...
        outs = self(hidden_states, references, references_num, memory_text, text_token_mask)
    ...
    def predict_by_feat(self,
                        ...
                        all_layers_outputs_numes: Tensor,
        ...
        num_preds = all_layers_outputs_numes[-1]
        sum_num_pred = num_preds.mean()
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            results = self._predict_by_feat_single(cls_score, bbox_pred, sum_num_pred,
                                                   token_positive_maps,
                                                   img_meta, rescale)
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor], references_num: List[Tensor], 
    ...
    def num_loss_by_feat_single(self, numes: Tensor, batch_gt_instances: InstanceList, batch_img_metas: List[dict]):
        num_imgs = numes.size(0)
        numes_list = [numes[i] for i in range(num_imgs)]
        weight = 1 / 10
        numes_gt = torch.zeros(num_imgs).to(numes.device)
        loss_num = 0
        for i, batch_gt_instance in enumerate(batch_gt_instances):
            bboxes = batch_gt_instance.bboxes

            numes_gt[i] = 0.5 + len(bboxes)*0.1

            if numes_gt[i] > 1:
                numes_gt[i] = 1

        for num_list, num_gt in zip(numes_list, numes_gt):
            sum_num_list = num_list.mean()
            loss = F.mse_loss(sum_num_list,  num_gt) * weight
    ...
    def loss_by_feat(
        ...
        loss_dict = super().loss_by_feat(
            ...
            enc_bbox_preds, batch_gt_instances, batch_img_metas, dn_meta, batch_gt_instances_ignore)
        num_denoising_queries = dn_meta['num_denoising_queries']
        if dn_meta is not None:
            all_layers_denoising_num_preds = \
                all_layers_outputs_numes[:, :, : num_denoising_queries, :]
            all_layers_matching_num_preds = \
                all_layers_outputs_numes[:, :, num_denoising_queries:, :]
        else:
            all_layers_denoising_num_preds = None
            all_layers_matching_num_preds = all_layers_outputs_numes
        
        losses_num = multi_apply(
                    self.num_loss_by_feat_single, all_layers_matching_num_preds,
                    batch_gt_instances=batch_gt_instances,
                    batch_img_metas=batch_img_metas)

        losses_num = losses_num[0]

        loss_dict['loss_num'] = losses_num[-1]

        for num_dec_layer, loss_num_i in enumerate(losses_num[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_num'] = loss_num_i
