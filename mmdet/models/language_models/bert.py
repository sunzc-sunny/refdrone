...
class BertModel(BaseModel):
    ...
    def forward(self, captions: Sequence[str], **kwargs) -> dict:
        ...
        input_ids = tokenized.input_ids
        # special tokens [101, 102, 1012, 1029]
        if self.use_sub_sentence_represent:
        ...
        # language_dict_features dict {embedded, masks, hidden}
        # print("language_dict_features", language_dict_features)
        if self.use_sub_sentence_represent:
            language_dict_features['position_ids'] = position_ids
            language_dict_features[
                'text_token_mask'] = tokenized.attention_mask.bool()
        # print('attention_mask', attention_mask.shape, attention_mask) # torch.Size([4, 15, 15])
        # print('mask', language_dict_features['masks'].shape, language_dict_features['masks']) # torch.Size([4, 15, 15])
        # print('text_token_mask', language_dict_features['text_token_mask'].shape, language_dict_features['text_token_mask']) # torch.Size([4, 15])
        # language_dict_features dict {embedded, masks, hidden, position_ids, text_token_mask}
        # print('attention_mask', attention_mask.shape, attention_mask, 'mask', language_dict_features['masks'].shape, language_dict_features['masks'], 'text_token_mask', language_dict_features['text_token_mask'].shape, language_dict_features['text_token_mask'])
